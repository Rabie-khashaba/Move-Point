...
class RepresentativeTarget extends Model
{
    ...
    /**
     * Get actual number of representatives converted in this month
     */
    public function getActualRepresentativesCountAttribute()
    {
        return Representative::whereYear('created_at', $this->year)
    ...
    public function giveBonusToQualifiedEmployees()
    {
        ...
        $salesEmployees = \App\Models\Employee::where('department_id', 7)
        ...
        foreach ($salesEmployees as $employee) {
            ...
            $employeeRepresentativesCount = Representative::where('created_by', $employee->user_id)
    ...
    private function addProgressiveBonusToEmployeeSalary($employee, $actualRepresentativesCount)
    {
        ...
        $qualifyingTargets = self::where('year', $this->year)
            ...
            ->where('representatives_count', '<=', $actualRepresentativesCount)
            ->orderBy('representatives_count', 'desc')
            ->get();
