...
class WhatsAppSimpleService
{
     ...
     * @param string|null $trainingDate تاريخ التدريب
    ...
    )
    {
        ...
        $formattedMessage = $this->formatMessage($message, $interviewDate, $googleMapUrl, $trainingDate);
     ...
     */
    private function formatMessage(string $message, ?string $interviewDate = null, ?string $googleMapUrl = null, ?string $trainingDate = null): string
    {
        $msg = "مرحباً 👋\n\n" . $message . "\n\n";

        if ($trainingDate) {
            $msg .= "📚 تاريخ التدريب: " . Carbon::parse($trainingDate)->format('d/m/Y H:i') . "\n";
            if ($googleMapUrl) $msg .= "📍 مكان التدريب:\n{$googleMapUrl}\n\n";
        }

        if ($interviewDate) {
            $msg .= "🗓️ تاريخ بدء العمل: " . Carbon::parse($interviewDate)->format('d/m/Y H:i') . "\n";
            if ($googleMapUrl) $msg .= "📍 المكان:\n{$googleMapUrl}\n\n";
        }
